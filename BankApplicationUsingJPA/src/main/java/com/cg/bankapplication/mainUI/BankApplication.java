				...
				do {
					...
					case 2: {
						...
						//	String time = dtf.format(now);
							Transaction trans = new Transaction("", "Credit",
					...
					//	pin = 0;
						...
						*/double newBalance = 0;
						...
						do {
							...
							try {
								...
								if (amount > 99 && amount < 50001) {
									...
									System.err.println(
											"The entered amount is incorrect.Please enter values between 100 and 50000.");
							} catch (InputMismatchException e) {
						...
						//LocalDateTime now = LocalDateTime.now();
						//String time = dtf.format(now);
						Transaction trans = new Transaction("", "Debit",
								service.getCustomerAccountDetails(accountNo).getCustomerId(), accountNo, 0, amount,
								"Withdraw");
						service.addTransaction(trans);
					...
					//	pin = 0;
						...
						if (service.showBalance(accountNo) >= amount) {
							if (service.fundTransfer(accountNo, accountCredit, amount)) {
								LocalDateTime now = LocalDateTime.now();
								//String time = dtf.format(now);
								Transaction trans = new Transaction("", "trnsfr",
										service.getCustomerAccountDetails(accountNo).getCustomerId(), accountNo,
										accountCredit, amount, "Fund transfer");
								service.addTransaction(trans);
					...
					case 5: {
						...
						if (transactions.isEmpty())
							...
							System.out.println("TransactionID\tTYPE\tCUSTID\tACCOUNTNO\tRECEIVERACCNO\tAMOUNT\tTRANSACTIONDETAILS\tTIME");
